...
## Contents
- `pairs_open_vins_estimator_plugin::OpenVins` — a `pairs_uav_managers::StateEstimator`
  plugin, built as the `PairsVinsEstimator_Plugin` library.
- `pairs_open_vins_estimator_plugin::OpenVins2` — a second variant, built as the
  `PairsVinsEstimator_Plugin2` library.
- Both are declared in `plugins.xml`.
...
## Install (ROS 2 Jazzy)
```bash
sudo apt install ros-jazzy-pairs-open-vins-estimator-plugin
```
